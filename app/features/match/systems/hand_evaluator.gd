...
static func evaluate(cards: Array) -> Dictionary:
	...
	for suited in suits:
		if suited.size() >= 5:
			var straight_flush_cards := best_straight_cards(suited)
			...
			var flush := result(5, card_ranks(flush_cards), flush_cards)
	...
	if not trips.is_empty():
		...
		if not other_pairs.is_empty():
			...
			return result(6, [trips[0], other_pairs[0]], trip_cards + pair_cards)
	if not best_flush.is_empty():
		return best_flush
	var straight_cards := best_straight_cards(cards)
...
static func best_straight_cards(cards: Array) -> Array:
	var by_rank: Dictionary = {}
	for card in cards:
		if not by_rank.has(card.rank):
			by_rank[card.rank] = []
	...
	for rank in by_rank.keys():
		...
		by_rank[rank] = ranked_cards
	var ranks: Array = by_rank.keys()
	var high := straight_high(ranks)
	if high == 0:
		return []
	var chosen: Array = []
	for offset in range(5):
		var target := high - offset
		var rank := 14 if target == 1 else target
		chosen.append(by_rank[rank][0])
	return chosen

static func result(category: int, kickers: Array, ordered_cards: Array = []) -> Dictionary:
	var score := category
